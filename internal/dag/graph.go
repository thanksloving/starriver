...
type Grapher interface {
	...
	Vertices() []Vertex
	Edges() []Edge
	EdgesTo(v Vertex) []Edge
	EdgesFrom(v Vertex) []Edge
	HasVertex(v Vertex) bool
	HasEdge(e Edge) bool
	Add(v Vertex) Vertex
	Remove(v Vertex) Vertex
	RemoveEdge(edge Edge)
	UpEdges(v Vertex) Set
	DownEdges(v Vertex) Set
	Connect(edge Edge)
	Subsume(other *Graph)
	StringWithNodeTypes() string
	String() string
}

// Vertex of the graph.
type Vertex = starriver.Node
...
func (g *Graph) RemoveEdge(edge Edge) {
	...
	if s, ok := g.downEdges[edge.Source().ID()]; ok {
		s.Delete(edge.Target())
	}
	if s, ok := g.upEdges[edge.Target().ID()]; ok {
...
func (g *Graph) Subsume(other *Graph) {
	...
	other.vertices.Filter(func(o GraphObject) bool {
		g.Add(o)
		return false
	})
	other.edges.Filter(func(o GraphObject) bool {
		g.Connect(o.(Edge))
		return false
	})
}

// StringWithNodeTypes String outputs some human-friendly output for the graph structure.
